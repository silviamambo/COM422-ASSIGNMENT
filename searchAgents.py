...
class CornersProblem(search.SearchProblem):
    ...
    def getStartState(self):
        ...
        "*** YOUR CODE HERE ***"
        usedNode = []
        return (self.startingPosition, usedNode)
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                if (nextx, nexty) in self.corners and (nextx, nexty) not in foundCorners:
                    usedNode = foundCorners + [(nextx, nexty)]
                    successors.append((((nextx, nexty), usedNode), action, 1))
                else:
...
def cornersHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    unvisitedCorners = []  # Corners not visited
    visitedCorners = state[1]  # Corners Visited
    node = state[0]  # Node
    heuristic = 0  # Value(Heuristic)
    # Corners not yet visited
    for corner in corners:
        if not corner in visitedCorners:
            unvisitedCorners.append(corner)
    # Find the sum of the shortest distances between the unvisitedCorners corners
