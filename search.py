...
def depthFirstSearch(problem):
    ...
    actionsX = []
    # Starting point
    border.push((problem.getStartState(), actionsX))
    while border:
        node, actions = border.pop()
        if not node in nodeVisited:
...
def uniformCostSearch(problem):
    ...
    border.push((problem.getStartState(), actionsX), problem)
    while border:
        node, actions = border.pop()
        if not node in visitedNode:
            visitedNode.append(node)
